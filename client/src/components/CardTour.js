...
const CardTour = ({imageFile, description, title, tags, _id, name, likes}) => {
    ...
    if (likes.length > 0) {
      return likes.find((like) => like === userId) ? (
        <>
          ...
          &nbsp;
          {likes.length > 2 ? (
            <MDBTooltip tag="a" title={`you are ${likes.length - 1} other likes`}>
              {likes.length} Likes
            </MDBTooltip>
          ) : (
  ...
  return (
    <MDBCardGroup>
      <MDBCardGroup className={"h-100 mt-2 d-sm-flex "} style={{maxWidth: "20rem"}}>
        <MDBCardImage
          src={imageFile}
          alt={title}
          position='top'
        ...
        <span className={"text-start tag-card"}>
          ...
          <MDBBtn style={{float: "right"}} tag="a" color="none"
            // onClick={handleLike}
